...
export default function KpiTotalDocuments({ 
  ...
  useEffect(() => {
    ...
    fetchWithAuth(apiUrl)
      ...
      .then((jsonData) => {
        setData(jsonData);
        calculateTotalDocuments(jsonData, effectiveStartDate, effectiveEndDate);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    calculateTotalDocuments(data, effectiveStartDate, effectiveEndDate);
  }, [data, effectiveStartDate, effectiveEndDate, dateField]);

  const calculateTotalDocuments = (documents, start, end) => {
    ...
    if (!start || !end) {
      // Si aucune période n'est sélectionnée, compter tous les documents
      ...
      const filteredDocuments = documents.filter(doc =>
        doc[dateField] >= startFormatted &&
        doc[dateField] <= endFormatted
      );
  ...
  return (
    <div className="visualisation relative w-64" data-id={id}>
      <div className="relative bg-white p-6 rounded-xl shadow-md flex flex-col items-start w-full">
        ...
        <p className="text-3xl font-bold text-black">{totalDocuments}</p>
