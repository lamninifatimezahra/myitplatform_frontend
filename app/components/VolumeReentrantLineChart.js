...
export default function VolumeReentrantsLineChart({
  ...
  // Fonction pour traiter les données des réentrants (version simplifiée pour line chart)
  const processReentrantData = (tickets, mode, currentSelectedValues = []) => {
    // Regrouper les périodes de chaque ticket et compter le total de réentrants par période
    const byId = {};
    ...
    // Compter le total des réentrants par période (tickets avec plus d'une itération)
    const result = {};
    Object.entries(byId).forEach(([ticketId, periods]) => {
      if (periods.length < 2) return; // ignorer non-réentrants
      const sorted = periods.sort((a, b) => a - b);
      const lastPeriod = sorted[sorted.length - 1]; // période de la dernière itération
      
      if (currentSelectedValues.length && !currentSelectedValues.includes(lastPeriod)) {
        return; // hors filtre
      }
      
      result[lastPeriod] = (result[lastPeriod] || 0) + 1;
    });

    setGroupedData(result);
  };

  // Fonction applyGlobalFilter
  ...
  // Préparer les données pour le graphique line chart
  ...
  const chartOptions = {
    ...
    scales: {
      x: {
        ticks: { 
          color: "black",
          maxRotation: filteredPeriodsData.length > 8 ? 45 : 0,
          font: {
            size: filteredPeriodsData.length > 12 ? 10 : 12
          }
        },
        title: {
          display: true,
      ...
      y: {
        ...
        title: {
          display: true,
          text: 'Nombre de Tickets Réentrants',
          color: "black"
        },
        grid: {
          color: "rgba(0, 0, 0, 0.1)"
        },
        grace: '20%' // Augmenter la marge en haut pour laisser de la place aux étiquettes
      },
    },
    interaction: {
      intersect: false,
