 ...
 * Renvoie un tableau à deux lignes :
 * - Ligne 1 : "DD/MM"
 * - Ligne 2 : "Sx" où x est le numéro de semaine.
 */
...
export default function GroupedBarChart() {
  ...
  if (viewMode === "day") {
    labels = selectedValues
      ...
      .map(d => formatDate(d)); // Chaque label sera un tableau : [ "DD/MM", "Sx" ]
  } else if (viewMode === "week") {
  ...
  // Fonction pour vérifier si une date est disponible dans le calendrier
  const isDateAvailable = (date) => {
    const dateStr = getLocalDateString(date);
    return availableDays.includes(dateStr);
  };

  // Render custom day contents (ici, on laisse le contenu par défaut)
