...
export default function UploadHispeedPage() {
  ...
  const hispeedInputRef = useRef(null);

  const handleBrowseHispeed = () => hispeedInputRef.current?.click();

  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      setHispeedFile(e.target.files[0]);
    }
  };

  const handleUpload = async () => {
    if (!hispeedFile) {
      setUploadMessage("Veuillez sélectionner un fichier Hispeed.");
    ...
    try {
      ...
      if (res.ok) {
        setUploadMessage(data.message || "Le fichier Hispeed a été uploadé avec succès.");
    ...
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex flex-1 justify-center items-center pt-28 p-6">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-[#31327e] mb-6 text-center">Upload Hispeed</h1>

          <div className="w-full space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Fichier Hispeed :</label>
              <div className="flex items-center gap-4">
                <input
                  type="text"
                  readOnly
                  value={hispeedFile ? hispeedFile.name : ""}
                  placeholder="Aucun fichier sélectionné"
                  className="flex-1 border border-gray-300 rounded-lg px-4 py-2 text-sm"
                />
                <button
                  type="button"
                  onClick={handleBrowseHispeed}
