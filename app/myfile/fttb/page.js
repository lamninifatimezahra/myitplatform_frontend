...
export default function UploadFTTBPage() {
  ...
  const fttbInputRef = useRef(null);

  const handleBrowseFTTB = () => fttbInputRef.current?.click();

  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      setFttbFile(e.target.files[0]);
    }
  };

  const handleUpload = async () => {
    if (!fttbFile) {
      setUploadMessage("Veuillez sélectionner un fichier FTTB.");
    ...
    try {
      ...
      if (res.ok) {
        setUploadMessage(data.message || "Le fichier FTTB a été uploadé avec succès.");
  ...
  Se déconnecter
      ...
      <div className="flex flex-1 justify-center items-center pt-28 p-6">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-[#31327e] mb-6 text-center">Upload FTTB</h1>

          <div className="w-full space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Fichier FTTB :</label>
              <div className="flex items-center gap-4">
                <input
                  type="text"
                  readOnly
                  value={fttbFile ? fttbFile.name : ""}
                  placeholder="Aucun fichier sélectionné"
                  className="flex-1 border border-gray-300 rounded-lg px-4 py-2 text-sm"
                />
                <button
                  type="button"
                  onClick={handleBrowseFTTB}
