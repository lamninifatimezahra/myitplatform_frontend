...
export default function UploadARTHIUSPage() {
  ...
  const arthiusInputRef = useRef(null);

  const handleBrowseARTHIUS = () => arthiusInputRef.current?.click();

  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      setArthiusFile(e.target.files[0]);
    }
  };
