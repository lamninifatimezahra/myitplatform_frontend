...
export default function Sidebar() {
  ...
  return (
    ...
    >
                ...
                <li key={dash}>
                  <Link
                    ...
                    className={`block px-2 py-1 rounded hover:text-[#4f72c3] ${
                      pathname === `/${dash}` ? 'font-bold text-[#4f72c3] bg-gray-100' : ''
                    }`}
                  >
                    {dash.toUpperCase()}
                  </Link>
      ...
      {/* Logos */}
      <div
        className={`flex justify-center items-center space-x-2 transition-all duration-300 ${
...
function SidebarItem({ icon, text, href, pathname, isOpen, isLogout = false, onClick }) {
  ...
  const commonClasses = `flex items-center px-3 py-2 rounded-lg transition-all duration-200 w-full ${
  ...
  if (isLogout) {
    return (
      <button
        onClick={onClick}
        className={commonClasses + ' text-red-500 hover:text-red-600 hover:bg-gray-100'}
        style={{ justifyContent: isOpen ? 'flex-start' : 'center', textAlign: 'left' }}
      >
        {icon}
