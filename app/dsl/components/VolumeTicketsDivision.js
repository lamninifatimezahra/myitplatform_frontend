...
export default function VolumeTicketsDivision() {
  ...
  // Couleurs homogènes pour toutes les divisions
  const colors = {
    "HLVIP": "#007bff",    // Bleu
    "FSC_DSI": "#1b2b6b",  // Bleu foncé
    "ADV_SC+": "#e6733f",  // Orange
    "BOT": "#7c4dd2",      // Violet
    "ANC-FTTB": "#c9b8f0", // Rose
    "DRCOGP": "#f1c40f",   // Jaune
    "SIC_CDP": "#f39c12",  // Orange foncé
    "SRR": "#2ecc71",      // Vert
  };
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/dashboard/api/dsl/data/`);
    ...
    datasets: [
      {
        ...
        backgroundColor: sortedDivisions.map(d => colors[d] || "#cccccc"), // Gris par défaut si couleur non définie
        borderWidth: 1,
        cutout: '40%',
        rotation: -90
      },
