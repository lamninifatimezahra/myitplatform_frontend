...
export default function SidebarMyAI({ sidebarOpen, setSidebarOpen }) {
...
function SidebarContent({ pathname, handleLogout, closeSidebar, user }) {
  const items = [
    ...
    { icon: <MdOutlineSupportAgent size={22} />, text: "Support Workflow", href: "#" },
    { icon: <AiOutlineRobot size={22} />, text: "Assistant MyAI", href: "#" },
    { icon: <AiOutlineSetting size={22} />, text: "Configuration", href: "#" },
  ];
...
function SidebarItem({ icon, text, href, pathname, onClick }) {
  ...
  const hoverClass = isInactive ? 'cursor-not-allowed text-gray-400' : 'text-gray-700 hover:text-[#31327e] hover:bg-gray-100';
