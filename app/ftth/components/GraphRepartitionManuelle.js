...
export default function GraphRepartitionManuelle({
  ...
  const toggleSelectAll = () => { if (viewMode === "day" || !selectedYear) return; setSelectedValues(allSelected ? [] : [...availablePeriodsForFilter]); };
  const toggleOne = (v) => { if (viewMode === "day") return; setSelectedValues((prev) => (prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v].sort((a, b) => a - b))); };
